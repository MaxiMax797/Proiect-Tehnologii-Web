...
function PersonalFolder() {
  ...
  const handleSubmit = (folderName_provided) => {
    let folderName = folderName_provided || 'New Folder';
    
    if (!folderName_provided) {
      const existingFolders = folders.map(folder => folder.title);
      let counter = 1;
  ...
  //   if (query) {
  //     axios.get(`http://localhost:8081/notite-api/search?query=${query}`)
  //       .then((res) => {
